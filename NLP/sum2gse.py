...
data_me =pd.read_csv('/content/drive/My Drive/my_nlp/sum.csv', sep='\t')['GSE_SUMMARY'].values.tolist()
df =pd.read_csv('/content/drive/My Drive/my_nlp/sum.csv', sep='\t')
mydict = dict(zip(df.GSE_SUMMARY, df.GSE))
#data_sum=pd.concat([data_me, data_you], ignore_index=True)

syn = pd.read_csv("/content/drive/My Drive/my_nlp/synonym.tsv", sep='\t')['synonym'].values.tolist()
syndf = pd.read_csv("/content/drive/My Drive/my_nlp/synonym.tsv", sep='\t')
...
for k in range(70001,137991):
  ...
  for i in matches:
    if(i[1] >=60):
      print(mydict[i[0]])
      num = num +1
      f = open("/content/drive/My Drive/my_nlp/nlp_me_71_.tsv", "a")
